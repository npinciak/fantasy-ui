...
export class DfsSlatesFacade extends GenericFacade({ selectorClass: DfsSlatesSelectors, actionHandler: DfsSlates }) {
  ...
  fetchSlates(site: string, sport: string): Observable<void> {
    return this.store.dispatch(new DfsSlates.Fetch({ site, sport }));
  }
}
