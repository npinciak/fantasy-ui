...
export class DfsNflChartSelector {
  @Selector([DfsNflPlayerSelectors.getPlayerTableData, DfsFilterSelector.slices.position])
  static getPlayerBarChartDataByStatAndPosition(
    playerTableData: NflDfsPlayerTableData[],
    position: string | null
  ): (stat: string) => ChartConfiguration['data'] {
    return (stat: string) => {
      const barChartData = transformTableDataToBarChartData(playerTableData, position ?? 'QB', stat);
