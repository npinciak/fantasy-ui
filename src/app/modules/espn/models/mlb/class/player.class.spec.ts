...
describe('[Class] Player', () => {
  ...
  it('should return player injury status', () => {
    expect(actual.injuryStatus).toBe(
      expected.playerPoolEntry.player.injuryStatus
    );
  });
  ...
  describe('playerOwnership', () => {
    it('should return ownership percentage', () => {
      expect(actual.playerOwnership.percentOwned).toBe(
        expected.playerPoolEntry.player.ownership.percentOwned
      );
    });

    it('should return ownership change', () => {
      expect(actual.playerOwnership.change).toBe(
        expected.playerPoolEntry.player.ownership.percentChange
      );
    });
  });
