...
describe('[Class] CurrentConditions', () => {
  ...
  const expected =
    MOCK_DATA.WEATHER_CURRENT_CONDITIONS.data.timelines[0].intervals[0].values;
  ...
  describe('weatherImg', () => {
    ...
    it('should return weather image', () => {
      ...
      expect(actual.weatherImg).toBe(
        'https://raw.githubusercontent.com/Tomorrow-IO-API/tomorrow-weather-codes/master/color/mostly_cloudy.svg'
      );
    });

    for (const code of weatherCodes) {
      it(`should append \'_day\' to ${WEATHER_MAP[code]
        .split(' ')
        .join('_')
        .toLowerCase()}`, () => {
        ...
        const img = `${WEATHER_MAP[code]
          .split(' ')
          .join('_')
          .toLowerCase()}_day.svg`;

        // eslint-disable-next-line max-len
        expect(actualClass.weatherImg).toBe(
          `https://raw.githubusercontent.com/Tomorrow-IO-API/tomorrow-weather-codes/master/color/${img}`
        );
      });
    }
  });
